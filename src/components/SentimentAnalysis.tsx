...
export function SentimentAnalysis({ currentData }: SentimentAnalysisProps) {
  ...
  const sentimentConfig = {
    positive: { icon: Smile, color: 'text-green-500', bg: 'bg-green-50', bar: 'bg-green-500' },
    neutral: { icon: Meh, color: 'text-yellow-500', bg: 'bg-yellow-50', bar: 'bg-yellow-500' },
    negative: { icon: Frown, color: 'text-red-500', bg: 'bg-red-50', bar: 'bg-red-500' },
  };
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-lg font-semibold text-gray-900">AI Sentiment</h3>
      </div>
      {loading ? (
        ...
        <div className="flex flex-col items-center">
          ...
          <span className={`text-sm font-medium capitalize ${config.color} mb-4`}>{sentiment}</span>
