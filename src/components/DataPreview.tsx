...
export function DataPreview({ data, onDataUpdate }: DataPreviewProps) {
  ...
  const handleCellSave = useCallback(() => {
    ...
    const updatedData: UploadedData = { ...data, rows: newRows };
    onDataUpdate(updatedData);
    setEditing(null);
  }, [editing, data, onDataUpdate]);

  const handleCellCancel = useCallback(() => setEditing(null), []);
          ...
          <div className="flex items-center gap-3">
            {onDataUpdate && (
              <span className="flex items-center gap-1 text-[10px] text-gray-400">
                <Edit3 size={10} /> Click cells to edit
              </span>
            )}
        ...
        {data.numericColumns.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {data.numericColumns.map((col) => (
              <span key={col} className="px-2 py-0.5 text-xs rounded-full bg-blue-100 text-blue-700">{col}</span>
            ))}
          </div>
        )}
        {data.categoricalColumns.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-1.5">
            {data.categoricalColumns.map((col) => (
              <span key={col} className="px-2 py-0.5 text-xs rounded-full bg-orange-100 text-orange-700">{col}</span>
            ))}
          </div>
        )}
      </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-gray-200">
              <th className="px-3 py-2 text-left font-medium text-gray-400 whitespace-nowrap w-8">#</th>
              {data.headers.map((header) => (
                <th key={header} className="px-3 py-2 text-left font-medium text-gray-500 whitespace-nowrap">{header}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {previewRows.map((row, i) => (
              <tr key={i} className="border-b border-gray-100 last:border-0 group">
                <td className="px-3 py-2 text-gray-400 text-[10px] select-none">{i + 1}</td>
                {data.headers.map((header) => {
                  ...
                  return (
                    <td
                      ...
                      className={`px-3 py-2 whitespace-nowrap transition-colors ${
                        isEditing ? 'p-0' : onDataUpdate
                          ? 'cursor-pointer hover:bg-blue-50 group-hover:ring-1 group-hover:ring-inset group-hover:ring-blue-200'
                          : ''
                      } ${isNumeric ? 'text-right tabular-nums text-gray-800 font-medium' : 'text-gray-700'}`}
                      ...
                      {isEditing ? (
                        <div className="flex items-center gap-0.5 px-1">
                          <input
                            ...
                            className="flex-1 bg-blue-50 border border-blue-400 rounded px-1.5 py-1 text-xs text-gray-900 outline-none min-w-[60px]"
                          />
                          <button onClick={handleCellSave} className="p-0.5 rounded hover:bg-blue-100 text-blue-600">
                            <Check size={12} />
                          </button>
                          <button onClick={handleCellCancel} className="p-0.5 rounded hover:bg-gray-100 text-gray-400">
                            <X size={12} />
                          </button>
                        </div>
                      ) : (
